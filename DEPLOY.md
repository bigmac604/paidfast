...
```bash
git remote add origin https://github.com/<your-username>/paidfast.git
git push -u origin main
```

## 2. Create the service (Railway shown; Render is equivalent)

1. railway.app → New Project → **Deploy from GitHub repo** → pick `paidfast`
...
| `X402_PAY_TO` | `0x1e0516223a85e685e0ba30c6b6a118d408973698` | now — owner payout wallet (X Layer) |
...
POST https://<your-domain>/api/invoices  → 402 challenge (when X402_MODE≠off)
```
...
```
https://<your-domain>/api/invoices
```

Note the two payment layers stay separate: x402 charges the *freelancer* 1 USDT
per invoice issued; the invoice itself is paid by their client on X Layer.
